...
func queryMessages(chanID, lastID int64) ([]Message, error) {
	msgs := []Message{}
	// err := db.Select(&msgs, "SELECT * FROM message WHERE id > ? AND channel_id = ? ORDER BY id DESC LIMIT 100", lastID, chanID)
	err := db.Select(&msgs, "select m.id, m.content, m.created_at, u.name, u.display_name, u.avatar_icon from message m inner join user u on u.id = m.user_id where m.id > ? and m.channel_id = ? order by m.id desc limit 100", lastID, chanID)
	return msgs, err
}
